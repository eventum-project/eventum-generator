...
- **Connected the formatter preview to the configured pipeline** — a formatter can be loaded from any output plugin and events can be loaded from the latest debugger run, while both panes keep accepting independent manual changes until an explicit load replaces them
