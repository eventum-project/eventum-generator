...
- **Made every Console split adjustable** — the dividers between formatter, event, result, timestamp, debugger and state panes can be dragged to give the current task more room; each tool starts from its existing proportions and keeps the adjusted split until the workspace is reloaded
- **Connected the formatter preview to the configured pipeline** — it automatically follows any output plugin's formatter, can apply edited settings back to that output, and can replace its sample events with the latest debugger run while keeping them editable
...
- **Showed formatter configuration errors in Studio** — an invalid formatter field made the project impossible to save without marking the field that needed correction; the error now appears under that field, including nested syslog and structured data values
...
- **Refused plugin configurations that break their declared rules** — field constraints and validators, as well as rules across fields such as TLS material without TLS enabled, were dropped when the API relaxed a plugin model to accept `${params.*}` placeholders, so Studio could save a configuration the generator rejected at startup. Every declared rule now answers over the API when its value is concrete and waits for load time only when that value still carries a substitution token. A configuration saved earlier that breaks such a rule opens in the file editor with the rule named, in place of a form built over a configuration no generator would run
